...
- 工具运行时与工具注册；
- CloudBase 账号与文档持久化；
- AI 协助、使用事件与后续按次数计费能力。

## 当前状态

截至 `2026-06-08`，仓库里已经存在这些正式能力：
...
- Journey 文档已经使用正式的 `projectId / documentId / revision` 持久化链路
- 已落地 `tool_documents`、`tool_document_revisions`、`tool_usage_events`
- 已有 billing / entitlements 领域契约与 payment integration 第一版实现

当前仍在进行中的事项：

- 支付编排的 provider 绑定校验加固
- 真正的次数扣减接入业务操作
- 第二个工具接入
- 全局服务设计助手
...
- [docs/billing-design.md](D:\knowledge\codex\design\docs\billing-design.md)：次数账本与支付编排设计
